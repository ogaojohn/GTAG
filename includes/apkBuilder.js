...
function javaversion(callback) {
    ...
    spawn.on('close', function (code) {
        ...
        if (javaVersion !== "" || openJDKVersion !== "") {
            if (javaVersion.includes("1.8.0_333") || openJDKVersion.includes("1.8.0_333")) {
                spawn.removeAllListeners();
                spawn.stderr.removeAllListeners();
                return callback(null, (javaVersion || openJDKVersion));
            } else return callback("Wrong Java Version Installed. Detected " + (javaVersion || openJDKVersion) + ". Please use Java 1.8.0_333", undefined);
